...
export class DifferenceValidatorService {
  ...
  public startNewGame(): void {
    ...
    this.http.post<IDifferenceImage>(Constants.SERVER_BASE_URL + Constants.API_NEWGAME_URL, differenceImage)
      .toPromise()
  ...
  public async sendClickInfo(mousePos: IClickInfo): Promise<number[]> {
    return new Promise<number[]>((resolve: Function) => {
      resolve(this.http.post<number[]>(Constants.SERVER_BASE_URL + Constants.API_DIFFVALIDATOR_URL, mousePos)
        .toPromise());
    });
  }
